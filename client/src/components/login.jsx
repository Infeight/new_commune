...
const Login = () => {
  ...
  let getlogin = async()=>{
   let alllogin = await fetch ('https://commune1.onrender.com/login',{headers:{accept:'application/json'}})
  ...
  logins.map(login=>{
    ...
    localStorage.setItem('followinglist1',JSON.stringify(followinglist2))
       ...
       fetch('https://commune1.onrender.com/logins', { method: 'post', headers: { "Content-Type": "application/json" }, body: JSON.stringify(user) })
 ...
 handlesignup_anim();
  ...
  const submitSignup = async()=>{
    ...
    await  fetch('https://commune1.onrender.com/signup', { method: 'post', headers: { "Content-Type": "application/json" }, body: JSON.stringify(signup) })
